...
* VM extensions tend to be named differently based on which OS type the extension is designed to run on. For instance, the extension called `CustomScriptExtension` for Windows machines is called `CustomScriptForLinux` on Linux machines. When you want to use a new extension on a Linux VM, you should probably check whether there's a separate Linux version or not.
